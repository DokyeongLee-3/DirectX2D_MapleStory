...
bool CColliderBox2D::Init()
{
	...
	m_Info.Length = Vector2(50.f, 60.f);
...
void CColliderBox2D::PostUpdate(float DeltaTime)
{
	...
	// 이때는 이미 부모인 SceneComponent의 Update로 인해
	// ColliderBox2D의 Transform 정보가 이미 갱신되어 있을 것이므로
	// 갱신된 Transform 정보(ex. WorldPos)를 바탕으로
	// Center, Min, Max를 잡아준다
	...
	m_Info.Length.x = GetWorldScale().x / 2.f;
	m_Info.Length.y = GetWorldScale().y / 2.f;
