...
{
	// 마우스와 충돌을 하고 있던 충돌체가 제거가 된다면 마우스와 충돌한 충돌체 정보를 nullptr로 바꿔준다.
	...
	// for문을 돌면서 같은 충돌체가 중복되게 CheckPrevColliderSection을
	// 호출하는걸 방지??
	for (; iter != iterEnd; ++iter)
	{
		...
		// 이전 프레임에 충돌되었던 충돌체들이 지금 내가 있는 Section에
		// 있는지를 판단한다. 만약 내가 이동해서 이전 프레임에 충돌체들과
		// 다른 Section에 있다면 이 안에서 그 충돌체들을 이전 충돌 목록에서 지워준다
