...
bool CSceneResource::LoadSequence2D(std::string& resultName, const char* FullPath)
{
	if (!CResourceManager::GetInst()->LoadSequence2D(resultName, FullPath, m_Scene))
		return false;
...
void CSceneResource::InsertAnimationSequence2D(CAnimationSequence2D* Sequence)
{
	CAnimationSequence2D* Seq = FindAnimationSequence2D(Sequence->GetName());
