...
void CGraphicShader::AddInputDesc(const char* Name,
	unsigned int Index, DXGI_FORMAT Fmt, 
	...
	unsigned int InstanceDataStep)
{
	...
	// 맨처음에 12바이트의 Pos를 넣어주면
	// AlignedByteOffset은 0, 그 다음 Color를 넣어주면
	// AlignedByteOffset은 12
...
{
	// 3번째 인자는 VertexShader 코드
	if (FAILED(CDevice::GetInst()->GetDevice()->CreateInputLayout(&m_vecInputDesc[0],
		(UINT)m_vecInputDesc.size(), m_VSBlob->GetBufferPointer(),
	...
	const TCHAR* FileName, const std::string& PathName)
{
	...
	// 쉐이더 코드를 파일로 읽어와서 컴파일하는 함수
	// m_VSBlob에 컴파일된 바이트코드를 얻어낸다
