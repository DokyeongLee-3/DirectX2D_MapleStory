...
bool CAnimationSequence2D::Load(const char* FullPath)
{
	...
	if (TexEnable)
	{
		...
		switch (ImageType)
		{
			// Sequence를 Load하면서 Texture를 Load하는 이유는
			// Sequence에서 사용하고 있는 Texture가 Load가 미리 안되어 있을수도
			// 있기 때문에 Sequence를 Load하면서 같이 Texture도 Load한다
			// 이미 Load되어 있으면 SceneResource나 TextureManager에서 Find해서
			// 이미 Load된 Texture를 바로 리턴해줄것이다
