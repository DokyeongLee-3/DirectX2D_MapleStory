...
public:
	...
	// m_WorldRot에 변화를 준다 -> 나의 자전에 변화를 준다
public:
...
private:
	...
	// 나의 자전 정보이자, 자식에게 물려줄 공전 정보
	Vector3 m_WorldRot;
	// 나의 공전(=부모의 자전) 정보와 나의 위치 정보
	// 만약 내 자식이 m_InheritRot = false이면 m_WorldPos까지는 
	// 추적해서 따라오지만, 나의 자전에 따라서 자식이 공전하지는 않을 것이라는 의미
	// 예를 들어 A(조부모)-B(부모)-C(자식) 이런 계층구조에 C가 m_InheritRotation = false인 상태라면
	// C는 B가 공전하면서 생긴 변하는 WorldPos는 추적할 것이지만, 따라서 회전은 안할 것이라는 의미다 
	Vector3 m_WorldPos;
